...
export const products: ProductEntry[] = [
  ...
  },
  {
    ...
    title: "Tideway",
    summary:
      "A research-grade Rust pipeline for scientific sensor networks whose ingest layer clears 14–17M samples/s single-threaded (~3.6M/s end-to-end through the log), durably buffers offline during partitions, and reconciles zones on reconnection using rateless IBLT set diffing at ~93 bytes per differing record, a cost that holds at 0.1% divergence independent of dataset size. Ships Ed25519-signed BPv7-style bundle transport, CRDT control-plane state, FedDrift-lineage (FLTA) drift detection, and partial-result query semantics, with zero `unsafe` code and 88.7% line coverage across ~710 tests.",
  ...
  },
  {
    ...
    title: "Lucubrum",
    summary:
      "A fully-deployed three-service platform (Node.js/TypeScript orchestrator + FastAPI/Python curriculum service + React PWA) that generates DAG-structured learning roadmaps, ranks and attaches YouTube resources deterministically, creates LLM-graded adaptive exercises, and tracks mastery with weighted scoring. All LLM outputs are schema-validated via Pydantic with retry logic and full audit logging; provider (Gemini/Claude/GLM) is swappable via config.",
    tags: ["Three-Service Platform", "Schema-Validated LLM", "Adaptive Learning", "DAG Roadmaps"],
    visualKey: "lucubrum",
    repoUrl: "https://github.com/simenzzz/lucubrum",
...
export const researchEntries: ResearchEntry[] = [
  ...
  },
  {
    ...
    summary:
      "A three-tier cascading WAF: ReDoS-safe regex screening → temperature-calibrated char-LSTM router → Mondrian conformal specialist trained on the uncertain slice. The cascade reaches 0.972 F1 on CSIC2010, escalating only 3.04% of requests to Tier 3, with a saliency-driven feedback loop that synthesizes new regex rules from neural attribution scores.",
    tags: ["Cascading Classifier", "Char-LSTM", "ReDoS-Safe Regex", "Conformal Prediction"],
    ...
    method:
      "Implemented ReDoS-safe regex screening, a temperature-calibrated character-level LSTM router, a Tier-3 MLP specialist trained on the uncertain band, Mondrian conformal abstention, and a saliency-driven regex-synthesis feedback loop.",
    finding:
